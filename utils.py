...
def train_loop(model, train_loader, val_loader, n_epochs, criterion, optimizer, device, using_dist):
    ...
    train_roc_auc_per_epoch = []
    val_accuracy_per_epoch = []
    val_loss_per_epoch = []
    val_roc_auc_per_epoch = []
    ...
    for epoch in range(n_epochs):
        ...
        for images, labels in train_loader:
            ...
            train_roc_auc += (roc_auc_score(labels.detach().cpu(), probabilities.detach().cpu())) 

        # Calculate training metrics
        avg_train_loss = train_loss / len(train_loader)
        train_accuracy = correct_train / total_train
        avg_train_roc_auc = train_roc_auc / total_train 
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                probabilities = F.softmax(outputs, dim=1)[:, 1]
                val_roc_auc += (roc_auc_score(labels.detach().cpu(), probabilities.detach().cpu())) 

        # Calculate validation metrics
        avg_val_loss = val_loss / len(val_loader)
        val_accuracy = correct_val / total_val
        avg_val_roc_auc = val_roc_auc / total_val

        # Store metrics for this epoch
        train_accuracy_per_epoch.append(train_accuracy)
        train_loss_per_epoch.append(avg_train_loss)
        train_roc_auc_per_epoch.append(avg_train_roc_auc)
        val_accuracy_per_epoch.append(val_accuracy)
        val_loss_per_epoch.append(avg_val_loss)
        val_roc_auc_per_epoch.append(avg_val_roc_auc)
    ...
    if using_dist:
        ...
        return model, {
            ...
            'Average train ROC AUC per epoch': average_across_gpus(train_roc_auc_per_epoch),
